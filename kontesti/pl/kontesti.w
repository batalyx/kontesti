...
sub read_config {
  ...
  while (my $row = <CONF>) {
    ...
    if ($key =~ /meta-\d/) {$val =~ tr/a-zåäö/A-ZÅÄÖ/;}
    if ($key) {
      $conf->{$key} = $val;
    }
  }
...
sub help_win() {
  ...
  my @@helplist = split("\n",q{
     ...
     <b>Meta-K</b>   keyer-mode          <b>CTRL-L</b>   ruudun päivitys
...
#-----------------------------------------------------------------------------
# siirrä kursoria oikealle riittävästi $position:n mukaan
...
sub clock {
  ...
  if ($tim[0] == 0) {
    ...
    # peruskisassa tasavartein dupelista tyhjäksi ja sanasta huomautus
...
sub points_and_multi {
  ...
  if ($contest =~ /perus-y/) {
    ...
    $points += 2;
     ...
     if ($msg =~ /^[0-9]{3} [A-ZÅÄÖ]{2}$/ and substr($msg,0,1) == 6) {
       $points += 2;
     } else {
       $points += 1;
     }
...
sub main {
  ...
  while (!$stopped) {
    ...
    if ($keybuf =~ m/^[a-zA-ZäöåÄÖÅ_]$/) {
      $keybuf =~ tr#a-zåäö_#A-ZÅÄÖ/#;
    }
